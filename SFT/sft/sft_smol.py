"""
SFT Training Script - SmolLM2-135M
Steps:
    1. Load SmolLM2-135M
    2. Test before SFT (on MPS for speed)
    3. Train with SFT (on CPU to avoid OOM)
    4. Test after SFT (on MPS for speed)
...
MODEL_PATH  = os.path.join(SCRIPT_DIR, "models", "HuggingFaceTB", "SmolLM2-135M")
...
model.to(DEVICE)
results_before = test_model(model, tokenizer, QUESTIONS, title="BEFORE SFT - SmolLM2-135M")
model.to("cpu")
...
results_after = test_model(sft_trainer.model, tokenizer, QUESTIONS, title="AFTER SFT - SmolLM2-135M")
...
model_save_path = os.path.join(SCRIPT_DIR, "models", "SmolLM2-135M-SFT")
